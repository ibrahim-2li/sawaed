...
<body class="bg-gray-100 text-gray-800">
        ...
        <div class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center gap-1 sm:gap-2">
                    <a href="{{ route('home') }}" class="text-gray-600 hover:text-primary-600 px-2 sm:px-3 py-2 rounded-md text-xs sm:text-sm font-medium whitespace-nowrap">عرض الموقع</a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-gray-600 hover:text-primary-600 px-2 sm:px-3 py-2 rounded-md text-xs sm:text-sm font-medium whitespace-nowrap">تسجيل الخروج</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto py-4 sm:py-10 px-2 sm:px-4 lg:px-8" x-data="{ 
        ...
        @if(session('success'))
            <div class="bg-blue-100 border border-blue-400 text-blue-700 px-3 sm:px-4 py-3 rounded relative mb-4 sm:mb-6 text-sm" role="alert">
                <span class="block sm:inline">{{ session('success') }}</span>
            </div>
        @endif

        @if($errors->any())
            <div class="bg-red-100 border border-red-400 text-red-700 px-3 sm:px-4 py-3 rounded relative mb-4 sm:mb-6 text-sm" role="alert">
                <ul>
        ...
        <div class="flex overflow-x-auto space-x-2 sm:space-x-4 space-x-reverse mb-6 border-b border-gray-200 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
            <button @click="activeTab = 'settings'" :class="{ 'border-primary-600 text-primary-600': activeTab === 'settings', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'settings' }" class="whitespace-nowrap py-3 sm:py-4 px-2 sm:px-3 border-b-2 font-medium text-xs sm:text-sm flex-shrink-0">
                الإعدادات العامة
            </button>
            <button @click="activeTab = 'services'" :class="{ 'border-primary-600 text-primary-600': activeTab === 'services', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'services' }" class="whitespace-nowrap py-3 sm:py-4 px-2 sm:px-3 border-b-2 font-medium text-xs sm:text-sm flex-shrink-0">
                الخدمات
            </button>
            <button @click="activeTab = 'projects'" :class="{ 'border-primary-600 text-primary-600': activeTab === 'projects', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'projects' }" class="whitespace-nowrap py-3 sm:py-4 px-2 sm:px-3 border-b-2 font-medium text-xs sm:text-sm flex-shrink-0">
                المشاريع
            </button>
            <button @click="activeTab = 'clients'" :class="{ 'border-primary-600 text-primary-600': activeTab === 'clients', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'clients' }" class="whitespace-nowrap py-3 sm:py-4 px-2 sm:px-3 border-b-2 font-medium text-xs sm:text-sm flex-shrink-0">
                العملاء
            </button>
            <button @click="activeTab = 'messages'" :class="{ 'border-primary-600 text-primary-600': activeTab === 'messages', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'messages' }" class="whitespace-nowrap py-3 sm:py-4 px-2 sm:px-3 border-b-2 font-medium text-xs sm:text-sm relative flex-shrink-0">
                الرسائل
                @if($contacts->where('is_read', false)->count() > 0)
                    <span class="absolute top-1 sm:top-2 -left-1 sm:-left-2 bg-red-500 text-white text-xs rounded-full h-4 w-4 sm:h-5 sm:w-5 flex items-center justify-center">
            ...
            <form action="{{ route('dashboard.settings.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-y-4 sm:gap-y-6 gap-x-4 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-6">
                        <h3 class="text-base sm:text-lg font-medium leading-6 text-gray-900 mb-3 sm:mb-4">القسم الرئيسي (Hero)</h3>
                    </div>

                    <div class="sm:col-span-6">
                        <label for="hero_title" class="block text-sm font-medium text-gray-700">العنوان الرئيسي</label>
                        <input type="text" name="hero_title" id="hero_title" value="{{ $settings->get('hero_title') }}" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>

                    <div class="sm:col-span-6">
                        <label for="hero_subtitle" class="block text-sm font-medium text-gray-700">العنوان الفرعي (الملون)</label>
                        <input type="text" name="hero_subtitle" id="hero_subtitle" value="{{ $settings->get('hero_subtitle') }}" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>

                    <div class="sm:col-span-6">
                        <label for="hero_description" class="block text-sm font-medium text-gray-700">الوصف</label>
                        <textarea name="hero_description" id="hero_description" rows="3" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">{{ $settings->get('hero_description') }}</textarea>
                    </div>

                    <div class="sm:col-span-6 border-t pt-3 sm:pt-4 mt-3 sm:mt-4">
                        <h3 class="text-base sm:text-lg font-medium leading-6 text-gray-900 mb-3 sm:mb-4">قسم من نحن (About)</h3>
                    </div>

                    <div class="sm:col-span-6">
                        <label for="about_title" class="block text-sm font-medium text-gray-700">العنوان</label>
                        <input type="text" name="about_title" id="about_title" value="{{ $settings->get('about_title') }}" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>

                    <div class="sm:col-span-6">
                        <label for="about_description_1" class="block text-sm font-medium text-gray-700">الوصف الأول</label>
                        <textarea name="about_description_1" id="about_description_1" rows="3" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">{{ $settings->get('about_description_1') }}</textarea>
                    </div>

                    <div class="sm:col-span-6">
                        <label for="about_description_2" class="block text-sm font-medium text-gray-700">الوصف الثاني</label>
                        <textarea name="about_description_2" id="about_description_2" rows="3" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">{{ $settings->get('about_description_2') }}</textarea>
                    </div>

                    <div class="sm:col-span-6">
                        <label for="about_image" class="block text-sm font-medium text-gray-700">صورة قسم من نحن</label>
                        @if($settings->get('about_image'))
                            <img src="{{ asset($settings->get('about_image')) }}" alt="About Image" class="mt-2 mb-2 h-24 sm:h-32 w-auto rounded-lg object-cover">
                        @endif
                        <input type="file" name="about_image" id="about_image" accept="image/*" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <input type="email" name="contact_email" id="contact_email" value="{{ $settings->get('contact_email') }}" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>

                    <div class="sm:col-span-3">
                        <label for="contact_address" class="block text-sm font-medium text-gray-700">العنوان</label>
                        <input type="text" name="contact_address" id="contact_address" value="{{ $settings->get('contact_address') }}" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>

                     <div class="sm:col-span-3">
                        <label for="contact_linkedin" class="block text-sm font-medium text-gray-700">لينكد إن</label>
                        <input type="text" name="contact_linkedin" id="contact_linkedin" value="{{ $settings->get('contact_linkedin') }}" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>
                    <div class="sm:col-span-3">
                        <label for="contact_phone" class="block text-sm font-medium text-gray-700">رقم الهاتف</label>
                        <input type="text" name="contact_phone" id="contact_phone" value="{{ $settings->get('contact_phone') }}" class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm text-sm border-gray-300 rounded-md p-2 border">
                    </div>

                </div>
                <div class="mt-4 sm:mt-6">
                    <button type="submit" class="w-full sm:w-auto inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                        حفظ الإعدادات
                    </button>
                </div>
            </form>
        ...
        <div x-show="activeTab === 'services'" class="space-y-4 sm:space-y-6">
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg p-4 sm:p-6">
                ...
                <form action="{{ route('dashboard.services.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mt-4">
                        <button type="submit" class="w-full sm:w-auto inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700">
                            إضافة
                        </button>
                    </div>
                </form>
                ...
                @if($contacts->count() > 0)
                    <ul role="list" class="divide-y divide-gray-200">
                        @foreach($contacts as $contact)
                            <li class="px-4 py-5 sm:px-6 {{ $contact->is_read ? 'bg-white' : 'bg-blue-50' }}" x-data="{ expanded: false }">
                                    ...
                                    <div class="flex-1 min-w-0 cursor-pointer" @click="expanded = !expanded">
                                        <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <p class="text-xs sm:text-sm text-gray-600 break-all">{{ $contact->email }}</p>
                                            </div>
                                            @if($contact->phone)
                                                <div class="flex items-center gap-2">
                                                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <p class="text-xs sm:text-sm text-gray-600">{{ $contact->phone }}</p>
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        <div x-show="expanded" class="mt-3">
                                            <div class="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200">
                                                <p class="text-xs sm:text-sm font-medium text-gray-700 mb-2">الرسالة:</p>
                                                <p class="text-xs sm:text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{{ $contact->message }}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex items-center justify-between sm:flex-col sm:items-end gap-2 sm:gap-3 sm:mr-4">
                                        <span class="text-xs text-gray-500">
                                            {{ $contact->created_at->diffForHumans() }}
                                        </span>
                                        <div class="flex space-x-2 space-x-reverse">
                                            <form action="{{ route('dashboard.contacts.toggle-read', $contact->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="text-sm {{ $contact->is_read ? 'text-gray-600 hover:text-gray-900' : 'text-primary-600 hover:text-primary-900' }}" title="{{ $contact->is_read ? 'تعيين كغير مقروء' : 'تعيين كمقروء' }}">
                                                    @if($contact->is_read)
                                            ...
                                            <form action="{{ route('dashboard.contacts.destroy', $contact->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذه الرسالة؟')">
                                                @csrf
                                                @method('DELETE')
