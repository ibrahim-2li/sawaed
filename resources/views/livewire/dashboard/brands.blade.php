<div class="space-y-4 sm:space-y-6">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg p-4 sm:p-6">
        <h3 class="text-base sm:text-lg font-medium leading-6 text-gray-900 mb-3 sm:mb-4">إضافة علامة تجارية جديدة</h3>
        <form wire:submit.prevent="store">
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                <div class="sm:col-span-3">
                    <label for="brand_name" class="block text-sm font-medium text-gray-700">اسم العلامة التجارية</label>
                    <input type="text" wire:model="name" id="brand_name"
                ...
                <div class="sm:col-span-3">
                    ...
                    <input type="number" wire:model="order" id="brand_order"
                        class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border">
                    @error('order')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                ...
                <div class="sm:col-span-6">
                    <label for="brand_logo" class="block text-sm font-medium text-gray-700">شعار العلامة
                        التجارية</label>
                    <input type="file" wire:model="logo" id="brand_logo" accept="image/*"
                        class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border">
                    @error('logo')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <ul role="list" class="divide-y divide-gray-200">
            @foreach ($brands as $brand)
                <livewire:dashboard.brand-row :brand="$brand" :key="$brand->id" />
            @endforeach
        </ul>
    </div>
</div>
