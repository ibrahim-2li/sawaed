        ...
        <form wire:submit.prevent="store">
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                <div class="sm:col-span-3">
                    ...
                    <input type="text" wire:model="title" id="service_title"
                ...
                <div class="sm:col-span-3">
                    ...
                    <select wire:model="color" id="service_color"
                ...
                <div class="sm:col-span-6">
                    ...
                    <textarea wire:model="description" id="service_description" rows="3"
                ...
                <div class="sm:col-span-6">
                    ...
                    <input type="file" wire:model="image" id="service_image" accept="image/*"
                        class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border">
                    @error('image')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                    <div wire:loading wire:target="image" class="text-sm text-gray-500 mt-1">جاري التحميل...</div>
                </div>
                <div class="sm:col-span-6">
                    <label for="service_icon" class="block text-sm font-medium text-gray-700">SVG Icon Code</label>
                    <textarea wire:model="icon" id="service_icon" rows="3"
                        class="mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border"></textarea>
                    @error('icon')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <ul role="list" class="divide-y divide-gray-200">
            @foreach ($services as $service)
                <livewire:dashboard.service-row :service="$service" :key="$service->id" />
            @endforeach
        </ul>
    </div>
</div>
